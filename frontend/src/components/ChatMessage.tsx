    ...
    <div className="text-sm leading-relaxed text-slate-800 dark:text-slate-200 prose prose-sm prose-slate dark:prose-invert max-w-none
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          table: ({ children }) => (
            <div className="not-prose overflow-x-auto my-3 rounded-lg border border-slate-200 dark:border-slate-700">
          ...
          th: ({ children }) => (
            <th className="px-3 py-2 text-left text-xs font-bold text-slate-700 dark:text-slate-200 whitespace-nowrap border-b border-slate-200 dark:border-slate-600">{children}</th>
          ),
          td: ({ children }) => {
            if (typeof children === "string") {
              return <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-300 border-t border-slate-100 dark:border-slate-700/50">{renderInlineTokens(children)}</td>;
            }
            return <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-300 border-t border-slate-100 dark:border-slate-700/50">{children}</td>;
          },
          tr: ({ children }) => (
